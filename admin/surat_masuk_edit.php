...
include "../include/session.php"; 
	...
	if($file_suratmasuk != "") {
	  ...
	  if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
					move_uploaded_file($file_tmp, 'file_suratmasuk/'.$nama_file_baru); //memindah file gambar ke folder tujuan
						
					  // jalankan query UPDATE berdasarkan ID file kita edit
					 $query  = "UPDATE tbl_suratmasuk SET nomor_surat = '$nomor_surat', asal_surat = '$asal_surat', tanggal_terima = '$tanggal_terima', tanggal_surat = '$tanggal_surat', 
					 perihal = '$perihal', keterangan = '$keterangan', ditujukan = '$ditujukan', status = '$status', oleh = '$oleh', file_suratmasuk = '$nama_file_baru'";
					  ...
					  if(!$result){
						...
						//silahkan ganti surat_masuk.php sesuai halaman yang akan dituju
				 ...
				 //jika file ekstensi tidak pdf maka alert ini yang tampil
					echo "<script>alert('Ekstensi File Hanya Boleh PDF');window.location='surat_masuk.php';</script>";
				}
	  } else {
		// jalankan query UPDATE berdasarkan ID  kita edit
		$query  = "UPDATE tbl_suratmasuk SET nomor_surat = '$nomor_surat', asal_surat = '$asal_surat', tanggal_terima = '$tanggal_terima', tanggal_surat = '$tanggal_surat', 
		perihal = '$perihal', keterangan = '$keterangan', ditujukan = '$ditujukan', status = '$status', oleh = '$oleh'";
		$query .= "WHERE id_surat = '$id_surat'";
		$result = mysqli_query($connect, $query);
